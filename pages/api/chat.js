...
// ۴. راه‌اندازی جدیدترین مدل فعال جمینای برای تولید جواب نهایی
    const chatModel = genAI.getGenerativeModel({ model: "gemini-3.5-flash" }); // 👈 کلید موفقیت اینجاست!

    const prompt = `
تو پشتیبان هوشمند و سخنگوی دارالترجمه رسمی ارس هستی. با لحنی مودبانه، حرفه‌ای و صمیمی به زبان فارسی پاسخ کاربر را بده.

قوانین بسیار مهم:
۱. مستقیماً جواب سوال کاربر را بده و به هیچ وجه از کلماتی مثل "سلام"، "درود" یا "خوش آمدید" استفاده نکن.
۲. صرفاً از اطلاعات زیر (Context) استفاده کن. اگر در اطلاعات زیر نبود، بگو پاسخ را نمی‌دانی یا کاربر را به تماس با پشتیبانی راهنمایی کن و اطلاعات غلط نساز.
